#!/bios-store3/ghhuang/miniconda3/envs/R3.6.0/bin/Rscript
startTime = Sys.time()
...
#packlib = "/bios-store3/ghhuang/miniconda3/envs/R3.6.0/lib/R/library"
packlib = "~/miniconda3/envs/python2/lib/R/library/"
R_LIBS_USER = packlib
...
#fdata[,"scaffold"]  = factor(fdata[,"scaffold"], levels=c("I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV"))
fdata[, "scaffold"] = factor(fdata[,"scaffold"], levels=c("CP047869.1", "CP047870.1", "CP047871.1", "CP047872.1", "CP047873.1", "CP047874.1", "CP047875.1"))
...
annotateScaffolds <- function(scaffolds=fdata[,"scaffold"], starts=fdata[,"frameStart"], ends=fdata[,"frameEnd"]) {
	...
	result[,"centromere"] = sapply(result[,"name"], function(x){
		switch(x,
			"I"	= (77251+77360)/2/1000000,
			"II"	= (451123+451234)/2/1000000,
			"III"	= (422097+422206)/2/1000000,
			"IV"	= (554532+554643)/2/1000000,
			"V"	= (245929+246038)/2/1000000,
			"VI"	= (226190+226299)/2/1000000,
			"VII"	= (60727+60839)/2/1000000,
			"VIII"	= (217322+217432)/2/1000000,
			"IX"	= (257665+257774)/2/1000000,
			"X" 	= (217322+217432)/2/1000000,
			"XI"	= (257665+257774)/2/1000000,
			"XII"	= (987127+987236)/2/1000000,
			"XIII"	= (1049554+1049664)/2/1000000,
			0)
		})
